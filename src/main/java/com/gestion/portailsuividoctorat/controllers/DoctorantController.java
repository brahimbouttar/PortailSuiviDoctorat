...
public class DoctorantController {
    ...
    public String createDemande(@ModelAttribute Demande demande,
                                @RequestParam(required = false) MultipartFile demandeManusFile,
                                @RequestParam(required = false) MultipartFile rapportTheseFile,
                                @RequestParam(required = false) MultipartFile rapportAntiPlagiatFile,
                                @RequestParam(required = false) MultipartFile rapportPublicationsFile,
                                @RequestParam(required = false) MultipartFile attestationsFile,
                                HttpSession session,
                                RedirectAttributes ra) {
        try {
            List<Doctorant> doctorants = doctorantService.findAllDoctorants();
            getDoctorantConnecte(session, doctorants).ifPresent(demande::setDoctorant);
            remplirDocumentsDemande(demande, demandeManusFile, rapportTheseFile, rapportAntiPlagiatFile,
                    rapportPublicationsFile, attestationsFile);
    ...
    private void remplirDocumentsDemande(Demande demande,
                                         MultipartFile demandeManusFile,
                                         MultipartFile rapportTheseFile,
                                         MultipartFile rapportAntiPlagiatFile,
                                         MultipartFile rapportPublicationsFile,
                                         MultipartFile attestationsFile) throws IOException {
        if (demandeManusFile != null && !demandeManusFile.isEmpty()) {
            demande.setDemandeManus(saveFile(demandeManusFile));
        }
        if (rapportTheseFile != null && !rapportTheseFile.isEmpty()) {
            demande.setRapportThese(saveFile(rapportTheseFile));
        }
        if (rapportAntiPlagiatFile != null && !rapportAntiPlagiatFile.isEmpty()) {
            demande.setRapportAntiPlagiat(saveFile(rapportAntiPlagiatFile));
        }
        if (rapportPublicationsFile != null && !rapportPublicationsFile.isEmpty()) {
            demande.setRapportPublications(saveFile(rapportPublicationsFile));
        }
        if (attestationsFile != null && !attestationsFile.isEmpty()) {
            demande.setAttestations(saveFile(attestationsFile));
        }
    }
